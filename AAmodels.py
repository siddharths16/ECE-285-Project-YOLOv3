...
from models_lpf import *        # Adding Anti-aliasing classes

ONNX_EXPORT = False


def create_modules(module_defs, img_size, arc,bpf=0):
    # Constructs module list of layer blocks from module configuration in module_defs
    num_blurs=0     # Addition by Rohit
        ...
        if mdef['type'] == 'convolutional':
            ...
            if bpf and int(mdef['stride'])>1 :
                modules.add_module('Conv2d', nn.Conv2d(in_channels=output_filters[-1],
                                                    ...
                                                    padding=pad,
                                                    bias=not bn))
                modules.add_module('DSLayer', Downsample(channels=filters,  stride=int(mdef['stride'])))
                num_blurs+=1
            else:
                modules.add_module('Conv2d', nn.Conv2d(in_channels=output_filters[-1],
                                                    out_channels=filters,
    ...
    print(f"Number of blurs added: {num_blurs}")
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), arc='default', bpf=0):
        super(Darknet, self).__init__()

        self.module_defs = parse_model_cfg(cfg)
        self.module_list, self.routs = create_modules(self.module_defs, img_size, arc, bpf=bpf)
