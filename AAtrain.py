...
import AAtest  # import test.py to get mAP after each epoch
...
def train():
    ...
    # Initialize model
    model = Darknet(cfg, arc=opt.arc, bpf=opt.AAmode).to(device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if opt.prebias:
            ...
            if not (opt.notest or (opt.nosave and epoch < 10)) or final_epoch:
                with torch.no_grad():
                    if opt.test_AA_shift > 0:
                        # pdb.set_trace()
                        results_mat, maps_mat = test.test(cfg,
                                                          data,
                                                          batch_size=batch_size,
                                                          img_size=opt.img_size,
                                                          model=model,
                                                          ...
                                                          AAshift=opt.test_AA_shift)
                        # pdb.set_trace()
                        results = results_mat[0]
                        maps = maps_mat[0]
                    else:
                        results, maps = test.test(cfg,
                                                  ...
                                                  conf_thres=0.001 if final_epoch and epoch > 0 else 0.1,  # 0.1 for speed
                                                  save_json=final_epoch and epoch > 0 and 'coco.data' in data)
            if final_epoch and opt.test_AA_resize and not opt.test_AA_shift and not opt.AAmode:
                assert opt.test_AA_resize < 256
                print("In AA resize routine in train.py")
                num_aa_rz = (416-opt.test_AA_resize)/32+1
                for it, iz in enumerate(range(opt.test_AA_resize, 416+32, 32)):
                    with torch.no_grad():
                        print(f"AA image size: {iz}")
                        results_resize, maps_resize = test_translation.AAperf_resize(cfg,
                                                                                     data,
                                                                                     batch_size=batch_size,
                                                                                     img_size=iz,
                                                                                     model=model, conf_thres=0.001)
                    AA_file_name = 'AA_resize.txt'
                    s_aa = ('%10s' * 2 + '%10.3g' * 6) % ('%g/%g' %
                                                          (it, num_aa_rz - 1), '%.3gG' % mem, *mloss, len(targets), iz)
                    with open(AA_file_name, 'a') as f:
                        # P, R, mAP, F1, test_losses=(GIoU, obj, cls)
                        f.write(s_aa + '%10.3g' * 7 % results_resize + '\n')
                plot_AA_results()
            if final_epoch and opt.test_AA_shift and not opt.test_AA_resize:
                print("In AA shift routine in train.py")
                AAS_file_name = 'AA_shift.txt'
                for AAsh in range(0, opt.test_AA_shift):

                    s_as = ('%10s' * 2 + '%10.3g' * 6) % ('%g/%g' % (AAsh,
                                                                     opt.test_AA_shift - 1), '%.3gG' % mem, *mloss, len(targets))
                    with open(AAS_file_name, 'a') as f:
                        # P, R, mAP, F1, test_losses=(GIoU, obj, cls)
                        f.write(s_aa + '%10.3g' * 7 % results_mat[AAsh] + '\n')
        # Write epoch results
...
if __name__ == '__main__':
    ...
    # i.e. weights/darknet.53.conv.74
    ...
    parser.add_argument('--hard-mining', type=bool,
                        default=False, help='Use Negative Hard Mining')
    parser.add_argument('--test_AA_resize', type=int,
                        default=320, help='Test resize operation on AA model')
